...
func _ready() -> void:
	...
	for child in get_children():
		_tile_map.set_cell(_tile_map.local_to_map(child.position), 0, _empty_tile_atlas_coord, 0)
...
func _update_status() -> void:
	if len(locks) == len(unlock_indices):
		for pos in bridge_positions:
			_tile_map.set_cell(_tile_map.local_to_map(pos), 0, _filled_tile_atlas_coord, 0)
		...
		for entity in _tile_map.get_children():
			if (entity is not Bridge) and (_tile_map.local_to_map(entity.position) in bridge_cells):
				...
				entity_tween.tween_property(entity, "modulate:a", 0., 0.5)
